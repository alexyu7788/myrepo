 ...
 */
void CPiCam::control_check_configuration(int min_gpu_mem)
{
   int gpu_mem = control_get_mem_gpu();
   int supported = 0, detected = 0;
   control_get_camera(&supported, &detected);
   if (!supported)
...
int CPiCam::control_parse_cmdline(RASPICAM_CAMERA_PARAMETERS *params, const char *arg1, const char *arg2)
{
	return 0;
}

void CPiCam::control_display_help()
{

}

int CPiCam::control_cycle_test(MMAL_COMPONENT_T *camera)
{
	return 0;
}
...
int CPiCam::control_set_all_parameters(MMAL_COMPONENT_T *camera, const RASPICAM_CAMERA_PARAMETERS *params)
{
   ...
   result  = control_set_saturation(camera, params->saturation);
   result += control_set_sharpness(camera, params->sharpness);
   result += control_set_contrast(camera, params->contrast);
   result += control_set_brightness(camera, params->brightness);
   result += control_set_ISO(camera, params->ISO);
   result += control_set_video_stabilisation(camera, params->videoStabilisation);
   result += control_set_exposure_compensation(camera, params->exposureCompensation);
   result += control_set_exposure_mode(camera, params->exposureMode);
   result += control_set_flicker_avoid_mode(camera, params->flickerAvoidMode);
   result += control_set_metering_mode(camera, params->exposureMeterMode);
   result += control_set_awb_mode(camera, params->awbMode);
   result += control_set_awb_gains(camera, params->awb_gains_r, params->awb_gains_b);
   result += control_set_imageFX(camera, params->imageEffect);
   result += control_set_colourFX(camera, &params->colourEffects);
//   result += control_set_thumbnail_parameters(camera, &params->thumbnailConfig);  //TODO Not working for some reason
   result += control_set_rotation(camera, params->rotation);
   result += control_set_flips(camera, params->hflip, params->vflip);
   result += control_set_ROI(camera, params->roi);
   result += control_set_shutter_speed(camera, params->shutter_speed);
   result += control_set_DRC(camera, params->drc_level);
   result += control_set_stats_pass(camera, params->stats_pass);
   result += control_set_annotate(camera, params->enable_annotate, params->annotate_string,
                                          ...
                                          params->annotate_x,
                                          params->annotate_y);
   result += control_set_gains(camera, params->analog_gain, params->digital_gain);
 ...
 */
int CPiCam::control_get_all_parameters(MMAL_COMPONENT_T *camera, RASPICAM_CAMERA_PARAMETERS *params)
{
	vcos_assert(camera);
	vcos_assert(params);
	...
	params->sharpness = control_get_sharpness(camera);
	params->contrast = control_get_contrast(camera);
	params->brightness = control_get_brightness(camera);
	params->saturation = control_get_saturation(camera);
	params->ISO = control_get_ISO(camera);
	params->videoStabilisation = control_get_video_stabilisation(camera);
	params->exposureCompensation = control_get_exposure_compensation(camera);
	params->exposureMode = control_get_exposure_mode(camera);
	params->flickerAvoidMode = control_get_flicker_avoid_mode(camera);
	params->awbMode = control_get_awb_mode(camera);
	params->imageEffect = control_get_image_effect(camera);
	params->colourEffects = control_get_colour_effect(camera);
	params->thumbnailConfig = control_get_thumbnail_config(camera);
	*/
	return 0;
}
 ...
 */
void CPiCam::control_dump_parameters(const RASPICAM_CAMERA_PARAMETERS *params)
{
	const char *exp_mode = cli_unmap_xref(params->exposureMode, exposure_map, exposure_map_size);
	const char *fl_mode = cli_unmap_xref(params->flickerAvoidMode, flicker_avoid_map, flicker_avoid_map_size);
	const char *awb_mode = cli_unmap_xref(params->awbMode, awb_map, awb_map_size);
	const char *image_effect = cli_unmap_xref(params->imageEffect, imagefx_map, imagefx_map_size);
	const char *metering_mode = cli_unmap_xref(params->exposureMeterMode, metering_mode_map, metering_mode_map_size);
 ...
 */
void CPiCam::control_set_defaults(RASPICAM_CAMERA_PARAMETERS *params)
{
	vcos_assert(params);
 ...
 */
int CPiCam::control_set_saturation(MMAL_COMPONENT_T *camera, int saturation)
{
	int ret = 0;
 ...
 */
int CPiCam::control_set_sharpness(MMAL_COMPONENT_T *camera, int sharpness)
{
   int ret = 0;
 ...
 * @return
 */
int CPiCam::control_set_contrast(MMAL_COMPONENT_T *camera, int contrast)
{
   int ret = 0;
 ...
 * @return 0 if successful, non-zero if any parameters out of range
 */
int CPiCam::control_set_brightness(MMAL_COMPONENT_T *camera, int brightness)
{
   int ret = 0;
 ...
 * @return 0 if successful, non-zero if any parameters out of range
 */
int CPiCam::control_set_ISO(MMAL_COMPONENT_T *camera, int ISO)
{
   if (!camera)
      return 1;
 ...
 * @return 0 if successful, non-zero if any parameters out of range
 */
int CPiCam::control_set_metering_mode(MMAL_COMPONENT_T *camera, MMAL_PARAM_EXPOSUREMETERINGMODE_T mode)
{
 ...
 * @return 0 if successful, non-zero if any parameters out of range
 */
int CPiCam::control_set_video_stabilisation(MMAL_COMPONENT_T *camera, int vstabilisation)
{
   if (!camera)
      return 1;
 ...
 * @return 0 if successful, non-zero if any parameters out of range
 */
int CPiCam::control_set_exposure_compensation(MMAL_COMPONENT_T *camera, int exp_comp)
{
   if (!camera)
      return 1;
 ...
 * @return 0 if successful, non-zero if any parameters out of range
 */
int CPiCam::control_set_exposure_mode(MMAL_COMPONENT_T *camera, MMAL_PARAM_EXPOSUREMODE_T mode)
{
 ...
 * @return 0 if successful, non-zero if any parameters out of range
 */
int CPiCam::control_set_flicker_avoid_mode(MMAL_COMPONENT_T *camera, MMAL_PARAM_FLICKERAVOID_T mode)
{
 ...
 * @return 0 if successful, non-zero if any parameters out of range
 */
int CPiCam::control_set_awb_mode(MMAL_COMPONENT_T *camera, MMAL_PARAM_AWBMODE_T awb_mode)
{
...
int CPiCam::control_set_awb_gains(MMAL_COMPONENT_T *camera, float r_gain, float b_gain)
{
 ...
 * @return 0 if successful, non-zero if any parameters out of range
 */
int CPiCam::control_set_imageFX(MMAL_COMPONENT_T *camera, MMAL_PARAM_IMAGEFX_T imageFX)
{
 ...
 * @return 0 if successful, non-zero if any parameters out of range
 */
int CPiCam::control_set_colourFX(MMAL_COMPONENT_T *camera, const MMAL_PARAM_COLOURFX_T *colourFX)
{
 ...
 * @return 0 if successful, non-zero if any parameters out of range
 */
int CPiCam::control_set_rotation(MMAL_COMPONENT_T *camera, int rotation)
{
	MMAL_STATUS_T ret;
 ...
 * @return 0 if successful, non-zero if any parameters out of range
 */
int CPiCam::control_set_flips(MMAL_COMPONENT_T *camera, int hflip, int vflip)
{
 ...
 * @return 0 if successful, non-zero if any parameters out of range
 */
int CPiCam::control_set_ROI(MMAL_COMPONENT_T *camera, PARAM_FLOAT_RECT_T rect)
{
 ...
 */
int CPiCam::control_zoom_in_zoom_out(MMAL_COMPONENT_T *camera, ZOOM_COMMAND_T zoom_command, PARAM_FLOAT_RECT_T *roi)
{
 ...
 * @return 0 if successful, non-zero if any parameters out of range
 */
int CPiCam::control_set_shutter_speed(MMAL_COMPONENT_T *camera, int speed_ms)
{
   if (!camera)
      return 1;

   return mmal_status_to_int(mmal_port_parameter_set_uint32(camera->control, MMAL_PARAMETER_SHUTTER_SPEED, speed_ms));
}
 ...
 * @return 0 if successful, non-zero if any parameters out of range
 */
int CPiCam::control_set_DRC(MMAL_COMPONENT_T *camera, MMAL_PARAMETER_DRC_STRENGTH_T strength)
{
...
int CPiCam::control_set_stats_pass(MMAL_COMPONENT_T *camera, int stats_pass)
{
   if (!camera)
      return 1;
 ...
 * @return 0 if successful, non-zero if any parameters out of range
 */
int CPiCam::control_set_annotate(MMAL_COMPONENT_T *camera, const int settings, const char *string,
...
int CPiCam::control_set_gains(MMAL_COMPONENT_T *camera, float analog, float digital)
{
...
int CPiCam::control_get_saturation(MMAL_COMPONENT_T *camera)
{
	return 0;
}
int CPiCam::control_get_sharpness(MMAL_COMPONENT_T *camera){
	return 0;
}
int CPiCam::control_get_contrast(MMAL_COMPONENT_T *camera)
{
	return 0;
}
int CPiCam::control_get_brightness(MMAL_COMPONENT_T *camera)
{
	return 0;
}
int CPiCam::control_get_ISO(MMAL_COMPONENT_T *camera)
{
	return 0;
}
MMAL_PARAM_EXPOSUREMETERINGMODE_T CPiCam::control_get_metering_mode(MMAL_COMPONENT_T *camera)
{
	MMAL_PARAM_EXPOSUREMETERINGMODE_T exposure = MMAL_PARAM_EXPOSUREMETERINGMODE_AVERAGE;
	return exposure;
}
int CPiCam::control_get_video_stabilisation(MMAL_COMPONENT_T *camera)
{
	return 0;
}
int CPiCam::control_get_exposure_compensation(MMAL_COMPONENT_T *camera)
{
	return 0;
}
//MMAL_PARAM_THUMBNAIL_CONFIG_T CPiCam::control_get_thumbnail_parameters(MMAL_COMPONENT_T *camera)
//MMAL_PARAM_EXPOSUREMODE_T CPiCam::control_get_exposure_mode(MMAL_COMPONENT_T *camera)
//MMAL_PARAM_FLICKERAVOID_T CPiCam::control_get_flicker_avoid_mode(MMAL_COMPONENT_T *camera)
//MMAL_PARAM_AWBMODE_T CPiCam::control_get_awb_mode(MMAL_COMPONENT_T *camera)
//MMAL_PARAM_IMAGEFX_T CPiCam::control_get_imageFX(MMAL_COMPONENT_T *camera)
//MMAL_PARAM_COLOURFX_T CPiCam::control_get_colourFX(MMAL_COMPONENT_T *camera)
 ...
 */
int CPiCam::control_get_mem_gpu(void)
{
   char response[80] = "";
   int gpu_mem = 0;
 ...
 * @param detected  None-zero if a camera has been detected
 */
void CPiCam::control_get_camera(int *supported, int *detected)
{
   char response[80] = "";
