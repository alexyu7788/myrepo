...
MMAL_STATUS_T CreateComponent(struct component* component, const char* name)
{
	...
	{
		component->output = (struct port_info*)malloc(sizeof(struct port_info) * component->comp->output_num);
		if (!component->output)
		{
			status = MMAL_ENOMEM;
...
//		component.output[idx].cam_obj = this;
		...
		out_port->userdata = (struct MMAL_PORT_USERDATA_T*)&component->output[idx];/* Used for callback of output port. */
