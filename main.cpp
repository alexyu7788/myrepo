...
void printf_usage()
{
	printf("Trainging process: \n\tfcws -T -s source_yuv_directory -k X -m output_model_filename -o ouput_yuv_directory\n");
	printf("Detection process: \n\tfcws -D -m input_model_filename\n");
}
...
int main(int argc, char *argv[])
{
    ...
    if (dotraining)
    {
    	if (!infolder.size() || !outfolder.size() || !model_name.size() || !first_k_coms)
    	{
    		printf_usage();
    		exit(-1);
    	}
	...
	if (fcws)
	{
		if (dotraining && fcws->LoadFiles(infolder))
		{
